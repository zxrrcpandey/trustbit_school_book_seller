...
class BookItemCreator(Document):
    ...
    def on_submit(self):
        """Enqueue item creation as background job"""
        self.db_set("status", "In Progress", update_modified=False)
        frappe.db.commit()

        frappe.enqueue(
            "trustbit_school_book_seller.trustbit_school_book.doctype.book_item_creator.book_item_creator.create_items_background",
            docname=self.name,
            queue="default",
            timeout=600,
            ...
            try:
                ...
                if item:
                    ...
                    if flt(row.opening_stock) > 0:
                        try:
                            self.create_stock_entry(item.name, row)
                ...
                error_msg = str(e)
                # Friendly message for duplicate ISBN race condition
                if "Duplicate entry" in error_msg and "isbn_barcode" in error_msg.lower():
                    error_msg = f"ISBN/Barcode {row.isbn_barcode} already exists (created by another process)"

                frappe.db.set_value("Book Class Detail", row.name, {
                    "creation_status": "Failed",
                    "remarks": error_msg[:200]
                }, update_modified=False)
                failed_count += 1
    ...
    def create_single_item(self, row):
        ...
        item_data = {
            ...
            "custom_isbn_barcode": row.isbn_barcode or None,
        ...
        # Only add barcode if ISBN is provided
        if row.isbn_barcode:
            item_data["barcodes"] = [{
...
def create_items_background(docname):
    """Background job: create items for a submitted Book Item Creator"""
...
def check_isbn_exists(isbn_barcode, exclude_doc=None):
    ...
    if exclude_doc:
        query += " AND bic.name != %s"
        params.append(exclude_doc)

    duplicate = frappe.db.sql(query, params, as_dict=True)
    ...
    for row in doc.class_details:
        if row.creation_status == "Created" and row.generated_item_code:
            ...
            items_data.append({
                ...
                "Subject": doc.subject,
                "Author": doc.author,
                "ISBN/Barcode": row.isbn_barcode or "",
